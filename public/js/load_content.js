$(document).ready(function () {
    ...
    $.get('../php/load_content.php', 'aplication/json', (res) => {
        ...
        function unPacking(object, index) {
            currentIndex = index;
            Object.keys(object).forEach(function (key) {
                content.splice(currentIndex, currentIndex > index ? 0 : 1, object[key]);
                currentIndex++;
            });
        }
        ...
        $(':input[type="text"], textarea').each(function () {
            if (typeof (content[index]) == "string" && content[index].charAt(0) == '\{') {
                unPacking(JSON.parse(content[index]), index);
            }
        ...
        $('#about_img').attr('src', 'data:' + about_img_name_type['Type'] + ';base64,' + about_img);
